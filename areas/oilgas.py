...
def get_offsets_oilgas(api, radius, axis):
    ...
    try:
        ...
        graphJSON_offset_wtr = json.dumps(dict(data=data_offset_wtr, layout=layout),
                                          cls=plotly.utils.PlotlyJSONEncoder)
        graphJSON_offset_oil_ci = json.dumps(dict(data=ci_plot(df_offsets, 'oil', header['api'], config.scl_oil[0][1], config.scl_oil[1][1], config.scl_oil[2][1]), layout=layout_),
                                             cls=plotly.utils.PlotlyJSONEncoder)
        graphJSON_offset_wtr_ci = json.dumps(dict(data=ci_plot(df_offsets, 'water', header['api'], config.scl_wtr[0][1], config.scl_wtr[1][1], config.scl_wtr[2][1]), layout=layout_),
                                             cls=plotly.utils.PlotlyJSONEncoder)
        graphJSON_offset_stm_ci = json.dumps(dict(data=ci_plot(df_offsets, 'steam', header['api'], config.scl_stm[0][1], config.scl_stm[1][1], config.scl_stm[2][1]), layout=layout_),
                                             cls=plotly.utils.PlotlyJSONEncoder)
